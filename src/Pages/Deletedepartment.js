...
function Track() {
  ...
  const deleteUser = () => {
    axios.get(`http://127.0.0.1:8000/api/delete_department/${id}`)
      .then(response => {
  ...
  return (
    ...
    <center>
        <h2><font color='green'>You can remove permanent the Department by click the below button</font></h2>
      {deletionResult && <p>{deletionResult}</p>}
      
      <button onClick={deleteUser}>Delete Department</button></center>
    </div></div></>
  );
}
