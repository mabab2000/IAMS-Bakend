...
function Track() {
  ...
  const deleteUser = () => {
    axios.get(`http://127.0.0.1:8000/api/delete_user/${id}`)
      .then(response => {
  ...
  return (
    ...
    <center>
        <h2><font color='blue'>You can remove permanent the user by click the below button</font></h2>
      {deletionResult && <p>{deletionResult}</p>}
      
      <button onClick={deleteUser}>Delete User</button></center>
    </div></div></>
  );
}
